...
This cache supports computation of frame transforms, velocities and velocity-product-accelerations.
It also supports [`configuration`](@ref), [`velocity`](@ref), and [`vpa`](@ref), for coordinates.
...
jacobians. 
It also supports [`configuration`](@ref), [`velocity`](@ref), [`vpa`](@ref), and [`jacobian`](@ref)
for coordinates.
